...
/**
 * ===================================================================
 * AUDIT HELPER: FULL CODEBASE SCANNER
 * Memindai seluruh source code .gs via GitHubOpsService untuk
 * mengekstraksi LOC, method signature, dependensi, dan deteksi string.
 * ===================================================================
 */
function runFullCodeAuditScan() {
  try {
    var files = GitHubOpsService.readAllSourceFiles();
    if (!files || files.length === 0) {
      Logger.log('{"status":"error","reason":"no_source_files_found"}');
      return;
    }

    var auditReport = {
      scan_timestamp: DateTimeUtils.formatUntukPrompt(DateTimeUtils.nowWIB()),
      total_files: files.length,
      files: []
    };

    var knownModules = [
      'Config', 'SpreadsheetGateway', 'IdGenerator', 'DateTimeUtils', 'AppLogger',
      'BudgetRepository', 'ChatHistoryRepository', 'DocumentationRepository',
      'FactsRepository', 'ReminderRepository', 'AckPatternsRepository',
      'TransactionRepository', 'WalletRepository', 'KnowledgeRepository',
      'TelegramService', 'LLMProviderService', 'GeminiProvider', 'GroqProvider',
      'OpenRouterProvider', 'WebSearchProviderService', 'GoogleSearchProvider',
      'TavilySearchProvider', 'ChangeDetector', 'ChatSpecialist', 'CodeAuditor',
      'FeatureArchitect', 'FinanceSpecialist', 'KnowledgeSpecialist',
      'MemorySpecialist', 'ProjectBrain', 'ReminderSpecialist', 'SelfAwareness',
      'SelfHealingSpecialist', 'UserProfileSpecialist', 'PatchValidator',
      'DocSyncSpecialist', 'KnowledgeSyncSpecialist', 'LLMIntelligence',
      'SoulSpecialist', 'SoulMemory', 'BehaviorEngine', 'SyncOrchestrator',
      'CommandRouter', 'Manager', 'IntentAnalyzer', 'WebhookHandler'
    ];
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      for (var l = 0; l < lines.length; l++) {
        ...
        if (m1) {
          methods.push(m1[1] + '(' + m1[2].trim() + ')');
          continue;
        }
        var m2 = line.match(/^(\w+)\s*\(([^)]*)\)\s*\{/);
        if (m2 && ['if', 'for', 'while', 'switch', 'catch', 'function'].indexOf(m2[1]) === -1) {
          methods.push(m2[1] + '(' + m2[2].trim() + ')');
        }
      }

      // Deteksi dependensi modul
      var dependencies = [];
      for (var m = 0; m < knownModules.length; m++) {
        var mod = knownModules[m];
        if (name.indexOf(mod) === -1 && content.indexOf(mod + '.') >= 0) {
          dependencies.push(mod);
        }
      }

      // Deteksi string mencurigakan (panjang > 25 karakter atau mengandung karakter non-ASCII)
      var suspiciousStrings = [];
      var stringMatches = content.match(/'([^'\\]|\\.)*'|"([^"\\]|\\.)*"/g) || [];
      for (var s = 0; s < stringMatches.length; s++) {
        var str = stringMatches[s].slice(1, -1);
        if (str.length > 25 || /[^\x00-\x7F]/.test(str)) {
          // Abaikan URL, regex, ID, timestamp format, dan keyword teknis
          if (str.indexOf('http') !== 0 && str.indexOf('{{') === -1 && str.indexOf('SELECT') !== 0) {
            suspiciousStrings.push(str.substring(0, 40));
          }
        }
      }
      ...
      auditReport.files.push({
        ...
        suspicious_strings_count: suspiciousStrings.length,
        sample_suspicious_strings: suspiciousStrings.slice(0, 5)
      });
    }

    var output = JSON.stringify(auditReport, null, 2);
    Logger.log(output);
    return output;
  } catch (err) {
    var errObj = { status: 'error', message: err.message, stack: err.stack };
    Logger.log(JSON.stringify(errObj));
