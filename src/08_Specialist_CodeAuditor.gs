...
var CodeAuditor = {
  ...
  _generateFixes: function(findings) {
    ...
    // STEP 1: Buat backup branch dulu
    var backupBranch = GitHubOpsService.createBackupBranch('audit-' + timestamp);

    // STEP 2: Validasi semua fixes SEBELUM commit apapun
    var validatedFixes = [];
    var rejectedFixes = [];
    ...
    // STEP 3: Jika ada yang ditolak, lapor ke user
    if (validatedFixes.length === 0) {
      var errorReply = '🚫 *Semua Patch Ditolak oleh Validator*\n\n';
      rejectedFixes.forEach(function(rf) {
        errorReply += PatchValidator.formatResult(rf.validation) + '\n\n';
      });
      errorReply += 'Tidak ada yang di-commit ke GitHub. ' +
                    'Coba jalankan audit lagi.';
      return errorReply;
    }
    ...
    // STEP 5: Commit semua yang lolos validasi
    var successCount = 0;
    var failCount = 0;
    var changeSummary = [];
    ...
    validatedFixes.forEach(function(item) {
      ...
      if (ok) {
        successCount++;
        changeSummary.push('✅ `' + item.fix.fileName + '`: ' +
          (item.fix.changes || []).join(', '));
      } else {
        failCount++;
        changeSummary.push('❌ `' + item.fix.fileName + '`: gagal commit');
      }

      Utilities.sleep(1000);
    });

    // STEP 6: Buat PR dengan info lengkap
    var prBody = '## Audit Auto-Fix (' + scope + ')\n\n' +
                 changeSummary.join('\n') + '\n\n';

    if (backupBranch) {
      prBody += '## Backup\nBranch backup: `' + backupBranch + '`\n' +
                'Untuk rollback: `git reset --hard ' + backupBranch + '`\n\n';
    }

    if (rejectedFixes.length > 0) {
      prBody += '## Fix yang Ditolak Validator\n';
      rejectedFixes.forEach(function(rf) {
        prBody += PatchValidator.formatResult(rf.validation) + '\n\n';
      });
    }

    prBody += '\n---\n_Generated by Code Audit Agent with Patch Validator_';
